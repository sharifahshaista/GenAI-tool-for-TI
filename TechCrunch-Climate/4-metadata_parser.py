...
if __name__ == "__main__":
    folder = "/Users/sharifahshaista/GenAI-for-TI/techcrunch-climate/techcrunch_crawl_output"  # Change this
    df = process_markdown_folder(folder)

    # Save CSV
    csv_path = "techcrunch_raw.csv"
    df.to_csv(csv_path, index=False)
    print(f"Processed {len(df)} articles and saved to {csv_path}")

    # Save JSON
    json_path = "techcrunch_raw.json"
    df.to_json(json_path, orient="records", force_ascii=False, indent=2)
    print(f"Processed {len(df)} articles and saved to {json_path}")
